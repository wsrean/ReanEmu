...
public:
    ...
    struct boss_halion_realAI : public ScriptedAI
    {
        ...
        void Reset()
        {
			...
			m_uiInvoTimer = 180*IN_MILLISECONDS;
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            ...
            switch (getStage())
            {
                ...
                case 0: //PHASE 1 PHYSICAL REALM
					...
					if (m_uiInvoTimer <= uiDiff)
                    {
						me->SummonCreature(NPC_COMBUSTION,3168.515f, 544.791f, 72.889f);
						me->MonsterTextEmote("Cuidado hay un circulo de Combustión ígnea!", 0, true);
                        m_uiInvoTimer = m_uiInvoTimer = 180*IN_MILLISECONDS;
                    } else m_uiInvoTimer -= uiDiff;
                ...
                case 2:
					...
					if (GameObject* pGoRing = pInstance->instance->GetGameObject(pInstance->GetData64(GO_FLAME_CREP)))
					pGoRing->SetPhaseMask(65535,true);
					//Ahora se queman los arboles < script HALION> WS-CORE.
					if (GameObject* pGO = GameObject::GetGameObject(*me, pInstance->GetData64(GO_ARBOL)))
						pInstance->HandleGameObject(pGO->GetGUID(),true);
					
					if (GameObject* pGO = GameObject::GetGameObject(*me, pInstance->GetData64(GO_ARBOL1)))
						pInstance->HandleGameObject(pGO->GetGUID(),true);
					
					if (GameObject* pGO = GameObject::GetGameObject(*me, pInstance->GetData64(GO_ARBOL2)))
									pInstance->HandleGameObject(pGO->GetGUID(),true);
					
					if (GameObject* pGO = GameObject::GetGameObject(*me, pInstance->GetData64(GO_ARBOL3)))
						pInstance->HandleGameObject(pGO->GetGUID(),true); 
                    break;
...
//        me->SetDisplayId(10045);
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (m_uiCorporealityTimer <= diff)
            {
                ...
                if (pInstance->GetData(TYPE_HALION_EVENT) != SPECIAL)
                    return;
        ...
        class spell_twilight_cutter_AuraScript : public AuraScript
        {
            ...
            void HandleTriggerSpell(AuraEffect const* /*aurEff*/)
            {
                ...
                for(Map::PlayerList::const_iterator i = players.begin(); i != players.end(); ++i)
                {
                    ...
                    {
                        if (!pPlayer) continue;
                        if (pPlayer->isGameMaster()) continue;
                        if (!pPlayer->HasAura(SPELL_TWILIGHT_ENTER)) continue;
                        if (pPlayer->isAlive())
                        {
                            float AB = caster->GetDistance2d(target)+1;
                            float BC = caster->GetDistance2d(pPlayer)+1;
                            float AC = target->GetDistance2d(pPlayer)+1;
                            float p = (AB + BC + AC)/2;
                            float DC = (2*sqrt(p*(p-AB)*(p-BC)*(p-AC)))/AB;
                            if (DC < 3.75f || DC > 52.0f)
                                target->CastSpell(pPlayer, SPELL_TWILIGHT_CUTTER, true);
                        }
                    }
                }
            }
